...
function migratePreviousUserDataDir() {
  try {
    const currentDir = app.getPath('userData');
    const previousName = ['Aud', 'ex'].join('');
    const previousDir = path.join(path.dirname(currentDir), previousName);
    if (previousDir === currentDir || !fs.existsSync(previousDir) || fs.existsSync(currentDir)) return;
    fs.cpSync(previousDir, currentDir, { recursive: true, errorOnExist: false });
  } catch (_) { /* best-effort migration */ }
}

migratePreviousUserDataDir();
...
//   • the AUDEL_DISABLE_GPU env var or the --disable-gpu CLI flag is present, or
...
function isGpuDisabled() {
  if (process.env.AUDEL_DISABLE_GPU) return true;
...
function createWindow() {
  ...
  // Closing the window should stop playback and exit the app like a regular desktop app.
  mainWindow.on('close', () => {
    isQuitting = true;
  });
...
ipcMain.handle('dialog:chooseImage', async () => {
  const { canceled, filePaths } = await dialog.showOpenDialog({
    properties: ['openFile'],
    filters: [{ name: 'Images', extensions: ['jpg', 'jpeg', 'jfif', 'png', 'webp', 'gif'] }]
  });
  if (canceled || filePaths.length === 0) return null;
  const filePath = filePaths[0];
  const stat = await fs.promises.stat(filePath);
  if (stat.size > 12 * 1024 * 1024) {
    return { error: 'Image is too large. Choose a file up to 12 MB.' };
  }
...
ipcMain.handle('app:setHardwareAcceleration', async (event, enabled) => {
  ...
  const choice = dialog.showMessageBoxSync(mainWindow, {
    ...
    detail: 'Перезапустить Audel сейчас?',
  });
  if (choice === 0) {
    isQuitting = true;
    app.relaunch();
...
function resolveDownloadsDir(requested) {
  const fallback = path.join(app.getPath('music'), 'Audel Downloads');
...
function httpsGetJson(url) {
  return new Promise((resolve, reject) => {
    const req = https.get(url, { headers: { 'User-Agent': 'Audel' } }, (res) => {
