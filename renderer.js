...
// Storage keys.
const LS = {
  libraryMeta: 'audel-library-meta',
  favorites: 'audel-favorites',
  playlists: 'audel-playlists',
  settings: 'audel-settings',
  recents: 'audel-recents',
  ytState: 'audel-dl-yt-state',
  ymState: 'audel-dl-ym-state',
  queue: 'audel-dl-queue',
  wavePeaks: 'audel-wave-peaks',
  playLog: 'audel-play-log',
};

const LEGACY_STORAGE_KEYS = {
  previousPrefix: atob('YXVkZXgt'),
  libraryMeta: atob('YW1iZXZvci1saWJyYXJ5LW1ldGE='),
  favorites: atob('YW1iZXZvci1mYXZvcml0ZXM='),
};

const PREVIOUS_LS = Object.fromEntries(
  Object.entries(LS).map(([key, value]) => [key, LEGACY_STORAGE_KEYS.previousPrefix + value.slice('audel-'.length)])
);
...
function migrateStorageKey(newKey, oldKey) {
  ...
  } catch (_) { /* ignore storage migration errors */ }
}

Object.keys(LS).forEach(key => migrateStorageKey(LS[key], PREVIOUS_LS[key]));
migrateStorageKey(LS.libraryMeta, LEGACY_STORAGE_KEYS.libraryMeta);
migrateStorageKey(LS.favorites, LEGACY_STORAGE_KEYS.favorites);
...
let settings = Object.assign({
  ...
  playbarTheme: 'auto',   // 'auto' follows the app theme; otherwise a theme id
  language: 'en',
  defaultFolder: '',
  scanSubdirs: true,
  downloads: false,
...
if (!settings.artistCovers || typeof settings.artistCovers !== 'object' || Array.isArray(settings.artistCovers)) {
  settings.artistCovers = {};
}
let recents = JSON.parse(localStorage.getItem(LS.recents) || '[]');

// ── Discord Rich Presence ──
// Discord Rich Presence is disabled until a client id is configured.
...
(function clearLegacyExternalButton() {
  const b0 = settings.discord.buttons[0];
  const oldLabel = ['git', 'hub'].join('');
  if (b0 && String(b0.label || '').toLowerCase() === oldLabel) {
    b0.label = '';
    b0.url = '';
    saveSettings();
  }
...
const coverCache = {};
let library = libraryMeta.map(t => ({ ...t, cover: t.customCover || coverCache[t.path] || null }));
...
window.matchMedia('(prefers-color-scheme: light)').addEventListener('change', () => {
  ...
  if (settings.playbarTheme === 'system') applyPlaybarTheme(settings.playbarTheme);
});
...
function playbarThemeLabel(id) {
  if (id === 'auto') return tr('theme.auto');
  return themeLabel(id);
}

function normalizePlaybarTheme(id) {
  if (id === 'auto') return 'auto';
  return THEME_OPTIONS.some(t => t.id === id) ? id : 'auto';
}
...
function applyPlaybarTheme(id) {
  const bar = document.querySelector('.playbar');
  if (!bar) return;
  const normalized = normalizePlaybarTheme(id);
  if (normalized === 'auto') {
    bar.removeAttribute('data-playbar-theme');
    return;
  }
  bar.setAttribute('data-playbar-theme', resolveThemeId(normalized));
}
applyPlaybarTheme(settings.playbarTheme);
...
const I18N = {
  ru: {
    ...
    'discord.previewListening': 'Слушает Audel',
    ...
    'trackMeta.compact': 'Компактный список',
    'trackMeta.full': 'Подробный список',
    'trackMeta.hideTip': 'Скрыть исполнителя и альбом',
    'trackMeta.showTip': 'Показать исполнителя и альбом',
    ...
    'downloads.yt.idle.text': 'Введите название — внизу появится список mp3, доступных для скачивания. Файлы сохраняются в папку по умолчанию (если она задана в настройках) или в «Audel Downloads», и автоматически добавляются в библиотеку.',
    ...
    'setting.playbarTheme': '\u0426\u0432\u0435\u0442 \u043d\u0438\u0436\u043d\u0435\u0439 \u043f\u0430\u043d\u0435\u043b\u0438',
    'setting.playbarThemeDesc': '\u041c\u0435\u043d\u044f\u0435\u0442 \u0442\u043e\u043b\u044c\u043a\u043e \u0444\u043e\u043d \u043f\u0430\u043d\u0435\u043b\u0438; \u043f\u0440\u043e\u0433\u0440\u0435\u0441\u0441 \u0438 \u0433\u0440\u043e\u043c\u043a\u043e\u0441\u0442\u044c \u043e\u0441\u0442\u0430\u044e\u0442\u0441\u044f \u0446\u0432\u0435\u0442\u0430 \u0430\u043a\u0446\u0435\u043d\u0442\u0430.',
    'theme.auto': '\u041a\u0430\u043a \u043f\u0440\u0438\u043b\u043e\u0436\u0435\u043d\u0438\u0435',
    ...
    'setting.clearLibrary': 'Очистить библиотеку',
    'setting.clearLibraryDesc': 'Удаляет список треков из приложения, но не удаляет файлы с диска.',
    'btn.clearLibrary': 'Очистить',
    'modal.clearLibrary.title': 'Очистить библиотеку?',
    'modal.clearLibrary.text': 'Все треки, избранное и треки в плейлистах будут убраны из приложения. Файлы на диске останутся на месте.',
    'modal.clearLibrary.confirm': 'Очистить',
  ...
  en: {
    ...
    'discord.previewListening': 'Listening to Audel',
    ...
    'downloads.yt.idle.text': 'Enter a name — a list of downloadable mp3 files will appear below. Files are saved to your default folder (if set in Settings) or to "Audel Downloads", and added to your library.',
    ...
    'setting.playbarTheme': 'Bottom panel color',
    'setting.playbarThemeDesc': 'Changes only the panel background; progress and volume keep the accent color.',
    'theme.auto': 'Same as app',
  ...
  de: {
    ...
    'discord.previewListening': 'Hört Audel',
    ...
    'downloads.yt.idle.text': 'Gib einen Namen ein — unten erscheint eine Liste herunterladbarer mp3-Dateien. Dateien werden im Standardordner (falls in den Einstellungen festgelegt) oder in „Audel Downloads" gespeichert und zur Bibliothek hinzugefügt.',
    ...
    'setting.playbarTheme': 'Farbe der unteren Leiste',
    'setting.playbarThemeDesc': 'Aendert nur den Hintergrund; Fortschritt und Lautstaerke behalten die Akzentfarbe.',
    'theme.auto': 'Wie App',
  ...
  fr: {
    ...
    'discord.previewListening': 'Écoute Audel',
    ...
    'downloads.yt.idle.text': "Entrez un nom — la liste des mp3 téléchargeables apparaîtra ci-dessous. Les fichiers sont enregistrés dans votre dossier par défaut (s'il est défini dans les paramètres) ou dans « Audel Downloads », et ajoutés à votre bibliothèque.",
    ...
    'setting.playbarTheme': 'Couleur du panneau inferieur',
    'setting.playbarThemeDesc': 'Change seulement le fond; progression et volume gardent la couleur d accent.',
    'theme.auto': 'Comme l application',
  ...
  uk: {
    ...
    'discord.previewListening': 'Слухає Audel',
    ...
    'downloads.yt.idle.text': 'Введіть назву — нижче з\'явиться список mp3, доступних для завантаження. Файли зберігаються в теку за замовчуванням (якщо її задано в налаштуваннях) або в «Audel Downloads», і додаються до бібліотеки.',
    ...
    'setting.playbarTheme': '\u041a\u043e\u043b\u0456\u0440 \u043d\u0438\u0436\u043d\u044c\u043e\u0457 \u043f\u0430\u043d\u0435\u043b\u0456',
    'setting.playbarThemeDesc': '\u041c\u0456\u043d\u044f\u0454 \u043b\u0438\u0448\u0435 \u0444\u043e\u043d \u043f\u0430\u043d\u0435\u043b\u0456; \u043f\u0440\u043e\u0433\u0440\u0435\u0441 \u0456 \u0433\u0443\u0447\u043d\u0456\u0441\u0442\u044c \u0437\u0430\u043b\u0438\u0448\u0430\u044e\u0442\u044c\u0441\u044f \u043a\u043e\u043b\u044c\u043e\u0440\u0443 \u0430\u043a\u0446\u0435\u043d\u0442\u0443.',
    'theme.auto': '\u042f\u043a \u0437\u0430\u0441\u0442\u043e\u0441\u0443\u043d\u043e\u043a',
...
function applyLanguage(lang) {
  ...
  applyTrackMetaVisibility();
}

function applyTrackMetaVisibility() {
  const compact = !!settings.trackMetaCompact;
  root.classList.toggle('track-meta-compact', compact);
  document.querySelectorAll('[data-track-meta-toggle]').forEach(btn => {
    btn.classList.toggle('is-active', compact);
    btn.setAttribute('aria-pressed', compact ? 'true' : 'false');
    btn.title = compact ? tr('trackMeta.showTip') : tr('trackMeta.hideTip');
    const label = btn.querySelector('[data-track-meta-label]');
    if (label) label.textContent = compact ? tr('trackMeta.full') : tr('trackMeta.compact');
  });
}
...
function renderTrackRow(track, displayIndex, queue, options = {}) {
  ...
  const rowIndexCell = reorder
    ? `<div class="trow-num"><span class="trow-drag" title="Drag row to reorder" aria-label="Drag row to reorder"><svg class="i" width="12" height="12"><use href="#i-list"/></svg></span><span>${numCell}</span></div>`
...
function cssCoverUrl(dataUrl) {
  return `background-image:url('${String(dataUrl || '').replace(/'/g, '%27')}')`;
}

function playlistCoverStyle(pl, fallback) {
  return pl.cover ? cssCoverUrl(pl.cover) : `background:${pl.color || fallback}`;
}

function applyCoverBox(el, dataUrl, fallback) {
  if (dataUrl) {
    el.style.background = '';
    el.style.backgroundImage = `url('${String(dataUrl).replace(/'/g, '%27')}')`;
  } else {
    el.style.backgroundImage = '';
    el.style.background = fallback || 'var(--panel-alt)';
  }
}

async function chooseCoverImageDataUrl() {
  if (!window.electronAPI || !window.electronAPI.chooseImage) return null;
  const picked = await window.electronAPI.chooseImage();
  if (!picked) return null;
  if (picked.error) throw new Error(picked.error);
  if (!picked.dataUrl) return null;
  return await normalizeCoverImage(picked.dataUrl);
}

function normalizeCoverImage(dataUrl) {
...
function setEditorCoverPreview(coverSrc, title, label) {
  const cover = $('editor-cover');
  if (coverSrc) {
    cover.style.backgroundImage = `url('${String(coverSrc).replace(/'/g, '%27')}')`;
    $('editor-cover-letter').textContent = '';
    $('editor-cover-tag').textContent = label || tr('editor.coverEmbed');
  } else {
    cover.style.backgroundImage = '';
    $('editor-cover-letter').textContent = (title || '?')[0];
    $('editor-cover-tag').textContent = tr('editor.noCover');
  }
}

async function applyTrackCustomCover(path, dataUrl) {
  const t = trackByPath(path);
  if (!t) return;
  if (dataUrl) {
    ...
    delete t.customCover;
    let embedded = coverCache[path] || null;
    if (!embedded && window.electronAPI && window.electronAPI.parseMetadata) {
      try {
        const fresh = await window.electronAPI.parseMetadata(path);
        if (fresh && fresh.cover) {
          embedded = fresh.cover;
          coverCache[path] = embedded;
        }
      } catch (_) {}
    }
    t.cover = embedded;
  ...
  renderRecents();
  if (currentTrackIndex === trackIndexByPath(path)) updateNowPlayingUI(t);
  if (currentView === 'artists') refreshArtistsCoversInPlace();
  if (currentView === 'artist-detail') renderArtistDetail(activeArtistName);
}
...
function isCurrentQueueForPaths(paths) {
  return currentQueue.length === paths.length
...
function reorderPlaylistTrack(playlistId, fromIndex, insertIndex) {
  ...
  if (oldPaths.join('\n') === pl.trackPaths.join('\n')) return;
  savePlaylists();
  const orderedTracks = pl.trackPaths.map(trackByPath).filter(Boolean);
  if (isCurrentQueueForPaths(oldPaths)) currentQueue = orderedTracks;
...
function wireReorderRowDrag(row, { kind, playlistId, index }) {
  ...
  row.addEventListener('dragstart', e => {
    ...
    if (e.dataTransfer) {
      ...
      e.dataTransfer.setDragImage(ghost, Math.min(e.offsetX || 24, row.offsetWidth - 1), Math.min(e.offsetY || 20, row.offsetHeight - 1));
...
function renderPlaylists() {
  ...
  if (playlists.length === 0) {
    ...
    playlists.forEach((pl, i) => {
      ...
      const coverStyle = playlistCoverStyle(pl, PL_COVERS[i % PL_COVERS.length]);
      card.innerHTML = `
...
function renderPlaylistDetail(plId) {
  ...
  $('pl-hero-letter').textContent = pl.cover ? '' : (pl.name || '?')[0];
  applyCoverBox($('pl-hero-cover'), pl.cover, pl.color || PL_COVERS[0]);
  const tracks = pl.trackPaths.map(trackByPath).filter(Boolean);
  ...
  $('btn-pl-edit').onclick = () => openEditPlaylistModal(pl.id);
  $('btn-pl-cover-reset').hidden = !pl.cover;
  $('btn-pl-cover').onclick = async () => {
    try {
      const cover = await chooseCoverImageDataUrl();
      if (!cover) return;
      pl.cover = cover;
      savePlaylists();
      renderPlaylistDetail(pl.id);
    } catch (err) { console.warn('cover pick failed:', err); }
  };
  $('btn-pl-cover-reset').onclick = () => {
    delete pl.cover;
    savePlaylists();
    renderPlaylistDetail(pl.id);
  };
...
function artistCoverKey(name) {
  return String(name || '').trim().toLowerCase();
}

function getArtistCustomCover(name) {
  return settings.artistCovers[artistCoverKey(name)] || null;
}

function setArtistCustomCover(name, dataUrl) {
  const key = artistCoverKey(name);
  if (!key) return;
  if (dataUrl) settings.artistCovers[key] = dataUrl;
  else delete settings.artistCovers[key];
  saveSettings();
}
...
function buildArtistsIndex() {
  ...
  for (const a of map.values()) {
    ...
    const autoCover = (a.tracks.find(t => t.cover) || {}).cover || null;
    const customCover = getArtistCustomCover(a.name);
    out.push({
      name: a.name,
      tracks: a.tracks,
...
function refreshArtistsCoversInPlace() {
  ...
  const byName = new Map(buildArtistsIndex().map(a => [a.name, a]));
  cards.forEach(card => {
    const avatar = card.querySelector('.artist-avatar');
    if (!avatar) return;
    const artist = byName.get(card.dataset.artist);
    const cover = artist && artist.cover ? artist.cover : null;
    if (cover) {
      avatar.style.backgroundImage = `url('${String(cover).replace(/'/g, '%27')}')`;
      const letter = avatar.querySelector('.artist-avatar-letter');
      if (letter) letter.remove();
    } else {
      avatar.style.backgroundImage = '';
      if (!avatar.querySelector('.artist-avatar-letter')) {
        avatar.innerHTML = `<span class="artist-avatar-letter">${escapeHtml(artistInitials(card.dataset.artist || ''))}</span>`;
      }
    }
  });
}
...
function renderArtistDetail(name) {
  ...
  const artistCoverReset = $('btn-artist-cover-reset');
  if (artistCoverReset) artistCoverReset.hidden = !artist.customCover;
  const artistCoverBtn = $('btn-artist-cover');
  if (artistCoverBtn) artistCoverBtn.onclick = async () => {
    try {
      const cover = await chooseCoverImageDataUrl();
      if (!cover) return;
      setArtistCustomCover(artist.name, cover);
      renderArtistDetail(artist.name);
    } catch (err) { console.warn('artist cover pick failed:', err); }
  };
  if (artistCoverReset) artistCoverReset.onclick = () => {
    setArtistCustomCover(artist.name, null);
    renderArtistDetail(artist.name);
  };
...
// Controller bound to a container element; manages wave/line DOM + paint state.
function makeWave(containerEl) {
  ...
  function rebuild() {
    ...
    lineFill = null;
    linePreview = null;
    containerEl.classList.toggle('wave', mode === 'wave');
    containerEl.classList.toggle('line-progress', mode === 'line');

    if (mode === 'line') {
      linePreview = document.createElement('div');
      linePreview.className = 'progress-line-preview';
      lineFill = document.createElement('div');
      lineFill.className = 'progress-line-fill';
      containerEl.append(linePreview, lineFill);
      paint();
      return;
    }

    const frag = document.createDocumentFragment();
    bars = peaksCache.map((h) => {
  ...
  function setMode(nextMode) {
    const normalized = nextMode === 'line' ? 'line' : 'wave';
    if (normalized === mode && containerEl.children.length) return;
    mode = normalized;
    rebuild();
  }
  ...
  function setPeaks(peaks, key) {
    peaksCache = peaks || [];
    if (key === builtKey && mode === 'wave') return;
    builtKey = key;
    if (mode === 'wave') rebuild();
    else paint();
  }
  ...
  function paint() {
    if (mode === 'line') {
      ...
      if (linePreview) linePreview.style.width = hover == null ? `${progress}%` : `${Math.max(progress, hover)}%`;
      return;
    }
  ...
  setMode(settings.progressStyle);

  return { setPeaks, setProgress, setHover, setMode };
}
...
function applyProgressStyle() {
  const mode = settings.progressStyle === 'line' ? 'line' : 'wave';
  settings.progressStyle = mode;
  playbarWave.setMode(mode);
  fsWave.setMode(mode);
  const btn = $('btn-progress-style');
  if (btn) {
    btn.classList.toggle('is-active', mode === 'line');
    btn.setAttribute('aria-pressed', String(mode === 'line'));
    btn.title = mode === 'line' ? 'Показать волны' : 'Показать полоску';
    const use = btn.querySelector('use');
    if (use) use.setAttribute('href', mode === 'line' ? '#i-progress-line' : '#i-waveform');
  }
}

function toggleProgressStyle() {
  settings.progressStyle = settings.progressStyle === 'line' ? 'wave' : 'line';
  saveSettings();
  applyProgressStyle();
}

applyProgressStyle();
const progressStyleBtn = $('btn-progress-style');
if (progressStyleBtn) progressStyleBtn.addEventListener('click', toggleProgressStyle);
  ...
  for (const e of playLog) {
    if (e.t >= w.start && e.t < w.end) {
      ...
      if (e.s >= REPORT_PLAY_SEC) {
        plays++;
        const cover = (trackByPath(e.p) || {}).cover || coverCache[e.p] || null;
...
function wireVolume(trackEl, orientation = 'horizontal') {
  let dragging = false;
  ...
  trackEl.addEventListener('mousedown', e => { dragging = true; setFromPointer(e); });
  window.addEventListener('mousemove', e => { if (dragging) setFromPointer(e); });
  window.addEventListener('mouseup', () => { dragging = false; });
}
wireVolume($('vol-track'));
wireVolume($('fs-vol-track'), 'vertical');
...
const fsVolumeBox = $('fs-volume');
const fsMuteBtn = $('fs-btn-mute');
let fsVolumeOpen = false;
function setFullscreenVolumeOpen(open) {
  fsVolumeOpen = open;
  if (fsVolumeBox) fsVolumeBox.classList.toggle('is-open', fsVolumeOpen);
  if (fsMuteBtn) fsMuteBtn.setAttribute('aria-expanded', String(fsVolumeOpen));
}
if (fsMuteBtn) {
  fsMuteBtn.addEventListener('click', e => {
    e.stopPropagation();
    setFullscreenVolumeOpen(!fsVolumeOpen);
  });
}
if (fsVolumeBox) {
  fsVolumeBox.addEventListener('mousedown', e => e.stopPropagation());
}
document.addEventListener('mousedown', e => {
  if (fsVolumeOpen && fsVolumeBox && !fsVolumeBox.contains(e.target)) {
    setFullscreenVolumeOpen(false);
  }
});
audio.volume = Math.max(0, Math.min(1, Number(settings.volume ?? 0.07)));
settings.volume = audio.volume;
saveSettings();
updateVolumeUI();
...
function openFullscreen() {
  ...
  fsAnimating = true;
  cancelCoverAnim();
  applyFullscreenQueueVisibility();
...
let fsQueueVisible = settings.fsQueueVisible !== false;

function applyFullscreenQueueVisibility() {
  const overlay = $('fullscreen-overlay');
  const btn = $('btn-fs-queue');
  overlay.classList.toggle('queue-hidden', !fsQueueVisible);
  if (btn) {
    btn.classList.toggle('is-active', fsQueueVisible);
    btn.title = fsQueueVisible ? 'Скрыть очередь' : 'Показать очередь';
  }
}

function toggleFullscreenQueue() {
  fsQueueVisible = !fsQueueVisible;
  settings.fsQueueVisible = fsQueueVisible;
  saveSettings();
  applyFullscreenQueueVisibility();
}
...
const fsQueueBtn = $('btn-fs-queue');
if (fsQueueBtn) fsQueueBtn.addEventListener('click', toggleFullscreenQueue);
applyFullscreenQueueVisibility();
...
function confirmDelete({ kind, payload, title, text, confirmLabel }) {
  ...
  $('btn-confirm-delete').textContent = confirmLabel || tr('btn.delete');
  $('confirm-modal').classList.add('active');
}
$('btn-cancel-delete').addEventListener('click', () => {
  $('confirm-modal').classList.remove('active');
  $('btn-confirm-delete').textContent = tr('btn.delete');
  pendingDelete = null;
});
...
function resetNowPlayingEmpty() {
  $('track-title').textContent = tr('np.empty.title');
  $('track-artist').textContent = '\u2014';
  $('mini-cover-wrapper').style.backgroundImage = '';
  $('mini-cover-letter').textContent = '';
  $('fs-cover').style.backgroundImage = '';
  $('fs-cover-letter').textContent = '';
  $('fs-backdrop').style.background = 'transparent';
  $('fs-title').textContent = tr('np.empty.title');
  $('fs-artist').textContent = '\u2014';
  $('fs-album').textContent = '';
  $('time-current').textContent = '0:00';
  $('time-total').textContent = '0:00';
  $('fs-time-current').textContent = '0:00';
  $('fs-time-total').textContent = '0:00';
  playbarWave.setProgress(0);
  fsWave.setProgress(0);
  const heart = $('btn-favorite');
  heart.classList.remove('active');
  heart.querySelector('use').setAttribute('href', '#i-heart');
  const fsFav = $('fs-btn-favorite');
  fsFav.classList.remove('active');
  fsFav.querySelector('use').setAttribute('href', '#i-heart');
  $('fs-fav-label').textContent = tr('btn.favorite');
...
function clearLibrary() {
  ...
  savePlaylists();
  saveRecents();
  resetNowPlayingEmpty();
  renderCounts();
  renderLibrary();
  renderFavorites();
  renderRecents();
  if (currentView === 'artists') renderArtists();
  else if (currentView === 'artist-detail') setView('artists');
  else if (currentView === 'playlist-detail') renderPlaylistDetail(activePlaylistId);
...
function setPlaylistModalMode(mode, pl) {
  const isEdit = mode === 'edit';
  editingPlaylistId = isEdit && pl ? pl.id : null;
  $('playlist-modal-title').textContent = isEdit ? 'Редактировать плейлист' : tr('modal.newPlaylist.title');
  $('btn-create-playlist').textContent = isEdit ? 'Сохранить' : tr('btn.create');
}
function openNewPlaylistModal() {
  setPlaylistModalMode('new');
...
function openEditPlaylistModal(plId) {
  const pl = playlists.find(p => p.id === plId);
  if (!pl) return;
  setPlaylistModalMode('edit', pl);
  $('new-playlist-name').value = pl.name || '';
  $('new-playlist-desc').value = pl.desc || '';
  $('new-playlist-modal').classList.add('active');
  setTimeout(() => $('new-playlist-name').focus(), 50);
}
...
$('btn-create-playlist').addEventListener('click', () => {
  ...
  if (editingPlaylistId) {
    ...
    $('new-playlist-modal').classList.remove('active');
    editingPlaylistId = null;
    if (activePlaylistId === pl.id && currentView === 'playlist-detail') renderPlaylistDetail(pl.id);
...
async function openMetadataEditor(path) {
  ...
  const existing = trackByPath(path);
  let meta = existing || {};
  // Refresh from disk for full tag set, while keeping Audel-only fields.
  const fresh = await window.electronAPI.parseMetadata(path);
  if (fresh && fresh.cover) coverCache[path] = fresh.cover;
  meta = { ...meta, ...fresh, customCover: existing ? existing.customCover : null };
  meta.cover = meta.customCover || (fresh && fresh.cover) || coverCache[path] || null;
  ...
  setEditorCoverPreview(meta.cover, meta.title, meta.customCover ? 'Пользовательская обложка' : tr('editor.coverEmbed'));
  $('btn-track-cover-reset').hidden = !meta.customCover;
...
$('btn-track-cover').addEventListener('click', async () => {
  if (!pendingMetadataPath) return;
  const status = $('editor-status');
  try {
    const cover = await chooseCoverImageDataUrl();
    if (!cover) return;
    await applyTrackCustomCover(pendingMetadataPath, cover);
    const t = trackByPath(pendingMetadataPath);
    setEditorCoverPreview(cover, t ? t.title : '', 'Пользовательская обложка');
    $('btn-track-cover-reset').hidden = false;
    status.textContent = 'Обложка сохранена';
    status.className = 'editor-foot-status ok';
  } catch (err) {
    status.textContent = err.message || 'Не удалось выбрать обложку';
    status.className = 'editor-foot-status error';
  }
});
$('btn-track-cover-reset').addEventListener('click', async () => {
  if (!pendingMetadataPath) return;
  const status = $('editor-status');
  await applyTrackCustomCover(pendingMetadataPath, null);
  const t = trackByPath(pendingMetadataPath);
  setEditorCoverPreview(t && t.cover, t ? t.title : '', t && t.cover ? tr('editor.coverEmbed') : tr('editor.noCover'));
  $('btn-track-cover-reset').hidden = true;
  status.textContent = 'Пользовательская обложка сброшена';
  status.className = 'editor-foot-status ok';
});
...
document.querySelectorAll('[data-track-meta-toggle]').forEach(btn => {
  btn.addEventListener('click', () => {
    settings.trackMetaCompact = !settings.trackMetaCompact;
    saveSettings();
    applyTrackMetaVisibility();
  });
});
...
function renderSettings() {
  ...
  const playbarThemeCurrent = $('playbar-theme-current');
  if (playbarThemeCurrent) playbarThemeCurrent.textContent = playbarThemeLabel(settings.playbarTheme);
  document.querySelectorAll('#playbar-theme-select .select-opt').forEach(o => {
    o.classList.toggle('active', o.dataset.playbarTheme === settings.playbarTheme);
  });
  renderAccentPalette();
  // Toggles
...
const playbarThemeSelect = $('playbar-theme-select');
if (playbarThemeSelect) {
  playbarThemeSelect.querySelector('.select-btn').addEventListener('click', e => {
    e.stopPropagation();
    playbarThemeSelect.classList.toggle('open');
  });
  document.addEventListener('click', e => {
    if (!e.target.closest('#playbar-theme-select')) playbarThemeSelect.classList.remove('open');
  });
  playbarThemeSelect.querySelectorAll('.select-opt').forEach(o => {
    o.addEventListener('click', () => {
      settings.playbarTheme = normalizePlaybarTheme(o.dataset.playbarTheme);
      saveSettings();
      playbarThemeSelect.classList.remove('open');
      applyPlaybarTheme(settings.playbarTheme);
      renderSettings();
    });
  });
...
const clearLibraryBtn = $('btn-clear-library');
if (clearLibraryBtn) {
  clearLibraryBtn.addEventListener('click', () => {
    confirmDelete({
      ...
      confirmLabel: tr('modal.clearLibrary.confirm'),
    });
  });
}
...
function buildDiscordActivity() {
  ...
  if (d.showCover) {
    ...
    if (cover) {
      activity.assets = {
        large_image: cover,
        large_text: (t.album || t.title || 'Audel').slice(0, 128),
      };
    }
  }
